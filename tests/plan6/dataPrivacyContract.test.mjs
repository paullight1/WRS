...
test('sensitive upload grants require active consent and server-owned object paths', () => {
  ...
  assert.match(upload, /const objectKey = `\$\{resolved\.user\.id\}/)
  assert.match(upload, /storageBucket: grant\.bucket/)
  assert.match(upload, /storagePath: grant\.path/)
  assert.match(upload, /mime|contentType/i)
  assert.match(upload, /size/i)
  assert.doesNotMatch(upload, /String\(body\.(?:path|bucket)/)
  assert.doesNotMatch(upload, /storage(?:Bucket|Path):\s*body\./)
...
test('quality scoring is deterministic and cannot be client-authoritative', () => {
  ...
  assert.match(submissions, /submitDataAsset/)
  assert.match(data, /serviceRpc\('wrs_submit_data_asset'/)
  assert.doesNotMatch(submissions, /const\s+\w+\s*=\s*body\.(?:qualityScore|approved)/)
  assert.doesNotMatch(submissions, /p_(?:quality_score|approved)\s*:/)
})
