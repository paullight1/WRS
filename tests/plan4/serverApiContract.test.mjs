...
const endpoints = [
  ...
  'server/routes/robot/onboarding.js',
  'server/routes/robot/onboarding/complete.js',
  'server/routes/robot/configuration.js',
  'server/routes/robot/passport.js',
  'server/routes/robot/passport/pdf.js',
  'server/routes/robot/passport/verify.js',
]
...
test('onboarding and configuration writes call the atomic service-role RPCs', () => {
  const onboarding = read('server/routes/robot/onboarding/complete.js')
  const configuration = read('server/routes/robot/configuration.js')
...
test('passport reads use authoritative projections and public verification is privacy-safe', () => {
  assert.match(read('server/routes/robot/passport.js'), /wrs_get_robot_passport/)
  assert.match(read('server/routes/robot/passport/verify.js'), /wrs_verify_robot_passport/)
...
test('passport PDF descriptors are signed and short-lived', () => {
  const source = read('server/routes/robot/passport/pdf.js')
