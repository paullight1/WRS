...
test('Plan 11 live-activation phase files and machine-readable evidence exist', () => {
  ...
  for (let phase = 1; phase <= 10; phase += 1) {
    ...
    assert.ok(docs.some((name) => name.startsWith(`phase-11.${phase}-`)), `phase 11.${phase}`)
  }
})
...
test('GO requires every required gate to be PASS', () => {
  ...
  const oneBlocked = { ...allPass, gates: allPass.gates.map((gate, index) => index === 0 ? { ...gate, status: 'EXTERNAL_BLOCKER' } : gate) }
...
test('staging probe is HTTPS-only and checks production browser security headers', () => {
  ...
  for (const header of ['content-security-policy','strict-transport-security','x-content-type-options','referrer-policy']) assert.match(source, new RegExp(header))
})
