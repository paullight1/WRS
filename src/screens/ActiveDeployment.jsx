...
import { Button, Card, Progress, SectionTitle, Stat, StatusDot } from '../components/ui.jsx'
    ...
    return (
      <AppShell title="Deployment monitoring unavailable" back avatar={false}>
        <StateView
          ...
          desc="WRS hides mock hours, performance, client ratings and earnings outside this workspace until the deployment service is authoritative."
          action={<Button to="/deploy">Back to deployments</Button>}
        />
      </AppShell>
    )
    ...
    <AppShell title={isHistory ? 'Deployment record' : 'Active deployment'} back avatar={false}>
      <section>
        <Card className="relative overflow-hidden p-0">
          <Worksite3D industry={d.industry} height={200} label={`${robot.name} in ${site.name} — ${site.task}`} />
        ...
        <Card className="p-card-padding">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-label-sm text-outline">{d.industry} · record</p>
              <div className="mt-2">
                <StatusDot t="outline" label={`${robot.name} — ${d.status}`} />
              </div>
            </div>
          </div>
        </Card>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <Stat label="hours" value={d.hours} icon="schedule" t="primary" />
          <Stat label="tasks" value={d.tasks.toLocaleString()} icon="task_alt" t="tertiary" />
          ...
          <Stat label="rating" value={legacy.rating} icon="star" t="secondary" />
          <Stat label="safety" value={legacy.safety} icon="health_and_safety" t="success" />
        </div>
      </section>
      ...
      <section>
        ...
        <Card className="p-card-padding">
          <p className="text-body-md text-on-surface-variant">
            Displayed revenue and deductions are values only and are not confirmed, payable or settled.
          </p>
        </Card>
      </section>
