...
export default function Register() {
  ...
  const continueDetails = () => {
    if (form.fullName.trim().length < 2 || !form.email.trim() || !form.phone.trim())
      return setError('Enter your name, email and international phone number.')
    setError('')
    setStep(2)
  }
    ...
    <div className="relative min-h-screen px-margin-page py-10">
      <div className="mx-auto w-full max-w-sm">
        ...
        <p className="mt-2 text-body-md text-on-surface-variant">
          {step === 1
            ? 'How we reach you and verify your account.'
            : 'Use a strong password and confirm the legal notices.'}
        </p>
        ...
        {step === 1 ? (
          <Card className="mt-6 space-y-4 p-card-padding">
            ...
            <Button full size="lg" trailingIcon="arrow_forward" onClick={continueDetails}>
              Continue
            </Button>
          </Card>
        ) : (
          <Card className="mt-6 space-y-4 p-card-padding">
            ...
            <label className="flex items-start gap-3 text-label-sm text-on-surface-variant">
              <input
                ...
                onChange={(e) => setForm((v) => ({ ...v, privacyAccepted: e.target.checked }))}
                className="mt-0.5"
              />
        ...
        <div className="mt-4">
          <Disclosure icon="shield">
            Registration creates a pending account. Email/phone verification is required before protected WRS features
            unlock.
          </Disclosure>
        </div>
