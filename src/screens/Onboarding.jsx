...
export default function Onboarding() {
  ...
  useEffect(() => {
    if (!draft) return
    setStep((current) => (current === 0 ? draft.step : current))
    setName(draft.name || 'WRS-Pro-001')
    setPersonality(draft.personality || 'Logical')
    setPackageSlug(draft.requestedPackageSlug || 'professional')
  }, [draft])
  ...
  const next = async () => {
    ...
    try {
      ...
      if (result.status === 'entitlement-required') {
        setMessage(`The ${result.packageSlug} package is not active on this account. Activate the entitlement before provisioning the robot.`)
        return
      }
      armWelcome()
    ...
    <div className="relative flex min-h-screen flex-col px-margin-page py-8">
      ...
      <div className="mx-auto w-full max-w-md">
        <div className="mb-6 flex items-center gap-3">
          ...
          <span className="text-label-sm text-outline">{step + 1}/{steps.length}</span>
        </div>
        ...
        {step === 1 && (
          <div>
            <h1 className="mb-4 font-headline-lg-mobile text-headline-lg-mobile text-on-surface">Select the package your robot should use</h1>
            ...
            <p className="mt-3 text-label-sm text-outline">Selecting a package here does not purchase or activate it. The server verifies the account entitlement at completion.</p>
          </div>
        )}
              ...
              <div className="flex justify-between gap-3">
                {eyeColors.map((color) => (
                  <button
                    ...
                    style={{ background: color, boxShadow: eye === color ? `0 0 18px ${color}` : 'none' }}
        ...
        {step === 5 && (
          <div className="text-center">
            ...
            <h1 className="font-headline-lg-mobile text-headline-lg-mobile text-on-surface">Ready to provision {name}</h1>
            ...
            <div className="mt-5 rounded-2xl border border-white/10 bg-white/[.03] p-4 text-left text-body-sm text-on-surface-variant">
              <p>Completion requests one atomic server transaction for entitlement validation, robot creation, configuration and passport projection.</p>
              <p className="mt-2">No Robot ID, passport, wallet or training entitlement is claimed until that transaction succeeds.</p>
            </div>
          </div>
        )}

        {message && <p role="alert" className="mt-5 rounded-xl border border-error/30 bg-error/10 p-3 text-label-sm text-error">{message}</p>}
