    ...
    <AppShell title="Event code" subtitle="Server-verified reward claims" back avatar={false}>
      <Card className="space-y-4 p-card-padding">
        <Field
          ...
          autoComplete="off"
        />
        <Button full loading={busy} disabled={code.trim().length < 8} onClick={redeem}>
          Redeem code
        </Button>
        <p className="text-label-sm text-outline">
          Codes are hashed server-side, expire on the server, and can only award the event policy configured by WRS.
          Reloading this page cannot reset expiry or claim limits.
        </p>
      </Card>
      {message && (
