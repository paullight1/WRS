...
export const browserAuthClient = {
  ...
  register: (input: RegistrationInput) => request<{ userId: string; challenges: { id: string; kind: VerificationKind }[] }>('/api/auth/register', { method: 'POST', body: JSON.stringify(input) }),
  ...
  resendVerification: (userId: string, kind: VerificationKind) => request<Json>('/api/auth/verification/resend', { method: 'POST', body: JSON.stringify({ userId, kind }) }),
  ...
  enrollMfa: () => request<{ enrollmentId: string; provisioningUri: string; recoveryCodes: string[] }>('/api/auth/mfa/enroll', { method: 'POST', body: '{}' }),
  ...
  disableMfa: (code: string) => request<Json>('/api/auth/mfa/disable', { method: 'POST', body: JSON.stringify({ code }) }),
}
