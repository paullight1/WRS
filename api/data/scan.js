...
export default functionHandler(async (request) => {
  ...
  if (request.headers.get('authorization') !== `Bearer ${expected}`) throw new HttpError(401, 'Unauthorized scan result.', 'unauthorized')
  const body = await readJson(request, 16_000)
  const assetId = String(body.assetId || '').trim()
  const scanStatus = String(body.scanStatus || '').trim().toLowerCase()
  if (!assetId || !['clean', 'infected', 'failed'].includes(scanStatus)) throw new HttpError(400, 'Invalid scan result.', 'invalid-scan')
