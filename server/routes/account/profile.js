import { updateAuthoritativeProfile } from '../../../api/_lib/account.js'
import { assertSameOrigin, functionHandler, HttpError, json, readJson, requireMethod } from '../../../api/_lib/http.js'
...
export default functionHandler(async (request) => {
  ...
  if (input.fullName.length < 2 || input.fullName.length > 120)
    throw new HttpError(400, 'Name must be 2–120 characters.', 'invalid-name')
  if (input.countryCode && !/^[A-Z]{2}$/.test(input.countryCode))
    throw new HttpError(400, 'Country code is invalid.', 'invalid-country')
